...
const STEPS = [
  { id: "venue", title: "الملعب", hint: "الاسم والتواصل" },
  { id: "place", title: "الموقع والصور", hint: "الخريطة والمعرض" },
  { id: "court", title: "المواصفات", hint: "الحجم ومدة الحجز" },
  { id: "prices", title: "المراجعة", hint: "الأسعار ثم النشر" },
] as const;
...
export default function OnboardingPage() {
  ...
  const [form, setForm] = useState({
    ...
    duration: 60,
    size: "FIVE_V_FIVE" as (typeof COURT_SIZES)[number],
    surface: "ARTIFICIAL_GRASS" as (typeof COURT_SURFACES)[number],
    setting: "OUTDOOR" as (typeof COURT_SETTINGS)[number],
    hasLights: true,
    dayPrice: 50,
    eveningPrice: 70,
    weekendPrice: 80,
  ...
  useEffect(() => {
    void (async () => {
      ...
      setForm((current) => ({ ...current, venueTypeId: current.venueTypeId || venueTypes[0]?.id || "" }));
    })();
  }, [router]);
  ...
  function canGoNext() {
    ...
    if (step === 2 && !form.resourceName.trim()) {
  ...
  async function uploadPhotos(venueId: string) {
    ...
    for (const photo of ordered) {
      const { signedUrl, publicUrl } = await ownerApi.upload(venueId, photo.file.type);
  ...
  async function onSubmit(event: FormEvent) {
    ...
    if (step !== STEPS.length - 1) {
      if (canGoNext()) setStep((value) => value + 1);
      return;
    }
    if (!canGoNext()) return;
    setLoading(true);
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#ecfdf5_0%,#f1f5f9_220px)]">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 pb-36">
        <div className="mb-6">
          <p className="text-xs font-bold uppercase tracking-widest text-brand-700">{STEPS[step].hint}</p>
          <h1 className="mt-1 font-display text-3xl font-extrabold text-slate-900">{STEPS[step].title}</h1>
        </div>
        ...
        <form className="space-y-5" onSubmit={onSubmit}>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        اختياري. ارفع حتى {MAX_PHOTOS} صور، أو أضفها لاحقاً من إعدادات الملعب.
                      </p>
                    </div>
                  </div>
          ...
          <div className="fixed inset-x-0 bottom-0 z-20 border-t border-slate-200 bg-white/95 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] backdrop-blur">
            <div className="mx-auto flex max-w-3xl gap-3">
              {step > 0 && (
                <Button type="button" variant="outline" className="min-w-28" onClick={() => setStep((value) => value - 1)}>
